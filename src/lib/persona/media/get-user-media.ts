...
export default async function getUserMedia(userMediaSchema: z.infer<typeof getUserMediaSchema>): Promise<ServiceResponse<IMediaEntity[] | null>> {
    try {
        ...
        const media = await MediaEntityModel.find(queryInput).sort({
            createdAt: -1
        }).skip(safeSkip ?? 0).limit(safeLimit ?? 0);
